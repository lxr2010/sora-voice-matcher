...
def find_best_match(new_entry, old_data_map, old_data_normalized_map, old_data_list, model, old_embeddings):
    """执行三步匹配策略来查找最佳匹配。"""
    ...
    # 3. 向量相似度搜索
    if model and old_embeddings is not None:
        new_embedding = model.encode(new_text, convert_to_tensor=True)
        # 计算余弦相似度
        cosine_scores = util.cos_sim(new_embedding, old_embeddings)[0]
        # 找到最高分
        top_result_index = torch.argmax(cosine_scores).item()
        top_score = cosine_scores[top_result_index].item()

        if top_score > VECTOR_SIMILARITY_THRESHOLD:
            return old_data_list[top_result_index], f'vector_search ({top_score:.2f})'
...
def main():
    ...
    for new_entry in new_data:
        ...
        processed_count += 1
        best_match, match_type = find_best_match(new_entry, old_data_map, old_data_normalized_map, old_data_list, model, old_embeddings)
